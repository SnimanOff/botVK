from database.models import Battles
...
def format_battle_state(battle, extra_msg=""):
    state = battle.state
    lines = [
        f"{state["enemy_name"]}",
        f"Враг {state["enemy_health"]}/{state["enemy_max_health"]} здоровья",
        f"",
        f"Вы {state["player_health"]}/{state["player_max_health"]} здоровья",
        f"Стойка: {"защита" if state["player_stance"] == "defend" else "обычная"}",
    ]
...
def get_room_kb(dungeon, room, player):
    kb = Keyboard(inline=True)
    needs_clearing = room["type"] in ("combat", "boss", "treasure", "shrine")
    is_cleared = room.get("cleared", False)

    if not needs_clearing or is_cleared:
        ...
        for ex in forward_exits:
            button_text = f"Комната {ex['y']} Этажа {ex['x']}"
            kb.add(Callback(button_text, payload={"cmd": "dungeon_move", "x": ex["x"], "y": ex["y"]}))
            kb.row()

    if not is_cleared:
        if room["type"] in ("combat", "boss"):
            kb.add(Callback("В бой", payload={"cmd": "start_combat"}))
            kb.row()
        elif room["type"] == "treasure":
            kb.add(Callback("Открыть", payload={"cmd": "treasure_open"}))
            kb.row()
        elif room["type"] == "shrine":
            kb.add(Callback("Помолиться", payload={"cmd": "shrine_use"}))
            kb.row()
...
def get_combat_kb(battle):
    kb = Keyboard(inline=True)
    
    kb.add(Callback("Атака", payload={"cmd": "combat_action", "action": "attack"}), color=KeyboardButtonColor.POSITIVE)
    kb.add(Callback("Защита", payload={"cmd": "combat_action", "action": "defend"}), color=KeyboardButtonColor.SECONDARY)
    kb.row()
    
    potions = battle.state.get("player_potions", [])
    for potion in potions[:3]:
        potion_label = potion.get("display_name", potion.get("name", "Зелье"))
        kb.add(Callback(
            f"{potion_label}",
    ...
    kb.add(Callback("Зелья", payload={"cmd": "feature", "id": "potions"}), color=KeyboardButtonColor.PRIMARY)
    kb.row()
...
async def edit_message(event, text, keyboard=None):
    logger.info("edit_message: text_len={}, has_kb={}", len(text), keyboard is not None)
    api = event.ctx_api
    peer_id = event.object.peer_id
    
    try:
        if hasattr(event.object, "conversation_message_id") and event.object.conversation_message_id:
            logger.info("edit_message: deleting msg_id={}", event.object.conversation_message_id)
        ...
        logger.warning("edit_message: delete failed: {}", e)
    ...
    try:
        ...
        logger.info("edit_message: sent, result={}", result)
    except Exception as e:
        logger.error("edit_message: send failed: {}", e)
        raise
...
async def shrine_use(event, player, payload):
    ...
    buff = {"stat": "attack", "value": 15, "duration": 3}
    await UserService.add_dungeon_buff(dung, buff)
    
    room = await DungeonService.get_current_room(dung)
    msg = f"Алтарь дарует силу, Атака +15 на 3 боя.\n\n{format_room(room)}"
    kb = get_room_kb(dung, room, player)
...
async def start_combat(event, player, dungeon):
    ...
    if not ok:
        await edit_message(event, "Не удалось начать бой")
        return
...
async def build_dungeon_room_kb(dungeon, room_data):
    ...
    if room_type == "treasure" and not is_cleared:
        ...
        if has_battle:
            keyboard.add(
                Callback("Атака", payload={"cmd": "combat_action", "battle_id": battle.id, "action": "attack"}),
                color=KeyboardButtonColor.POSITIVE
            )
            keyboard.add(
                Callback("Блок", payload={"cmd": "combat_action", "battle_id": battle.id, "action": "defend"}),
            ...
            keyboard.add(
                Callback("Атаковать", payload={"cmd": "start_combat"}),
                color=KeyboardButtonColor.POSITIVE
            )
